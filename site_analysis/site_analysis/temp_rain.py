...
class WeatherData:
    ...
    def __init__(self, stn_numbers, bom_dir, weights):
        ...
        # Check first if data is available for all station numbers, otherwise delete missing from list
        missing_idx = []
        for i, stn_number in enumerate(stn_numbers):
            zip_file1 = self.get_station_file(stn_number, 
                                             os.path.join(self.bom_dir, 
                                                          self.high_temp_directory)
                                             )

            zip_file2 = self.get_station_file(stn_number, 
                                             os.path.join(self.bom_dir, 
                                                          self.low_temp_directory)
                                             )

            zip_file3 = self.get_station_file(stn_number, 
                                             os.path.join(self.bom_dir, 
                                                          self.rain_directory)
                                             )
            print(stn_number, zip_file1, zip_file2, zip_file3)
            if (not zip_file1) | (not zip_file2) | (not zip_file3):
                print('Missing station file:', stn_number)
                missing_idx.append(i)

        if len(missing_idx) > 0:
            stn_numbers = np.delete(stn_numbers, missing_idx)
            weights = np.delete(weights, missing_idx)
